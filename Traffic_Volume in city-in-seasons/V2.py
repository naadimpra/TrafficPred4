...
y_train_pred = scaler.inverse_transform(np.concatenate((X_train.reshape(-1, 4), y_train_pred), axis=1))[:, -1]
y_test_pred = scaler.inverse_transform(np.concatenate((X_test.reshape(-1, 4), y_test_pred), axis=1))[:, -1]
...
plt.title('Traffic Volume Forecasting using RNN - Training and Prediction')
plt.legend()

# Save the plot as an image
plt.savefig('traffic_volume_forecast_training_prediction.png')
...
plt.plot(data['date_time'][-365*2:], data['traffic_volume'][-365*2:], label='True Traffic Volume', color='blue')
